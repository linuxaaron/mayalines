...
const ALIASES = new Map([
  ...
  ["COLERIDGE, S. TAYLOR", "Samuel Taylor Coleridge"],
  ...
  ["BURNS, ROBERT", "Robert Burns"],
  ["BROWNING, ROBERT", "Robert Browning"],
  ["BROWNING, ELIZABETH B.", "Elizabeth Barrett Browning"],
  ["WILDE, OSCAR", "Oscar Wilde"],
  ["TENNYSON, ALFRED", "Alfred Tennyson"],
  ["CARLYLE, THOMAS", "Thomas Carlyle"],
  ["RUSKIN, JOHN", "John Ruskin"],
  ...
  ["MONTAIGNE, MICHEL DE", "Michel de Montaigne"],
  ["LA ROCHEFOUCAULD", "François de La Rochefoucauld"],
  ["GOETHE, WOLFGANG VON", "Johann Wolfgang von Goethe"],
...
for (const block of authorBlocks) {
  ...
  for (const text of paragraphs) {
    ...
    if (/^(act|scene|book|chapter|stanza|line|page|chapter|part)\b/i.test(text)) continue;
    if (/^(from|see|compare|note|ibid|source|chapter|book)\b/i.test(text)) continue;
    ...
    quotes.push({
      ...
      attributionStatus: "source-traced",
      copyrightStatus: "historical-source-review-required",
      slug: `${author.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${quotes.length + 1}`,
    });
...
if (quotes.length < 500) {
  throw new Error(`Only extracted ${quotes.length} usable quotes; refusing to create an incomplete 500-quote dataset.`);
}
