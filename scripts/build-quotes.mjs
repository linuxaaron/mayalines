...
for (const letter of LETTERS) {
  ...
  for (const paragraph of paragraphs) {
    ...
    if (/^[—–-]/.test(paragraph)) {
      if (pending) {
        const author = cleanQuoteText(paragraph.replace(/^[—–-]\s*/, "").replace(/[.\s]+$/, ""));
        const quote = cleanQuoteText(pending);
        ...
        if (author && quote.length >= 15 && quote.length <= 1500 && !seen.has(key)) {
          ...
          quotes.push({ id, quote, author, category: normalizeCategory(category), source: url, sourceName: SOURCE_NAME, sourceCommit: SOURCE_COMMIT, attributionStatus: "verified", copyrightStatus: "cleared", indexable: true, slug: `${slugify(quote).slice(0, 90)}-${id}` });
        }
      }
      pending = null;
...
if (quotes.length !== TARGET_COUNT) throw new Error(`Expected exactly ${TARGET_COUNT} verified public-domain quotes; extracted ${quotes.length}. Publication aborted.`);
