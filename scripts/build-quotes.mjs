...
const MIN_DESIRED_COUNT = 10000;
const USER_AGENT = "MayalinesQuoteBuilder/3.2 (+https://mayalines.com)";
...
// Researched public-domain Project Gutenberg quotation/maxim/aphorism sources.
// Multi-author anthologies are parsed only when an explicit attribution is present.
const GUTENBERG_SOURCES = [
  ...
  { id: 26107, name: "Maxims for Revolutionists", language: "en", author: "George Bernard Shaw" },
  { id: 47406, name: "Aphorismen zur Lebensweisheit", language: "de", author: "Arthur Schopenhauer" },
  { id: 77889, name: "Gesammelte Schriften: Aphorismen, Parabeln, Märchen und Gedichte", language: "de", author: "Marie von Ebner-Eschenbach" },
  ...
  { id: 75555, name: "Aphorismes du temps présent", language: "fr", author: "Gustave Le Bon" },
  { id: 16149, name: "Máximas Morales en Ilocano y Castellano", language: "es", author: "Anonymous" },
  { id: 68079, name: "Maximes", language: "nl", author: "François de La Rochefoucauld" },
];

const LETTERS = "ABCDEFGHIJKLMNOPRSTUVWYZ".split("");

const decode = (value) => value.replace(/&#39;|&#x27;/g, "'").replace(/&quot;/g, '"').replace(/&amp;/g, "&").replace(/&#(?:32|160);/gi, " ").replace(/&#(?:8203|8204|8205|65279);/gi, "").replace(/&(?:nbsp|NewLine);/gi, " ");
const clean = (value) => decode(String(value ?? "")).replace(/\u00a0/g, " ").replace(/[\u200B-\u200D\uFEFF]/g, "").replace(/[ \t]+/g, " ").replace(/\s*\n\s*/g, " ").trim();
...
function addQuote({ quote, author, category = "Wisdom", source, sourceName, sourceCommit, language = "en", id, slug, attributionStatus = "verified", copyrightStatus = "cleared", indexable = true }) {
  ...
  if (/project gutenberg|transcriber|table of contents|copyright|ebook|proofread/i.test(quote)) return;
...
async function safeFetchText(url) {
  try { const response = await fetch(url, { headers: { "User-Agent": USER_AGENT } }); if (!response.ok) throw new Error(`HTTP ${response.status}`); return await response.text(); }
  catch (error) { console.warn(`Skipping source ${url}: ${error instanceof Error ? error.message : String(error)}`); return ""; }
}

try { const existing = JSON.parse(await readFile("data/quotes.json", "utf8")); for (const quote of existing) addQuote({ ...quote, language: quote.language || "en" }); }
catch (error) { console.warn(`Could not seed existing quote corpus: ${error instanceof Error ? error.message : String(error)}`); }

for (const letter of LETTERS) {
  if (quotes.length >= TARGET_COUNT) break;
  const url = `${WIKISOURCE.url}/${letter}`;
  const html = await safeFetchText(url); if (!html) continue;
...
for (const source of GUTENBERG_SOURCES) {
  ...
  const fetched = await safeFetchText(url); if (!fetched) continue;
...
if (published.length < MIN_DESIRED_COUNT) console.warn(`Corpus currently contains ${published.length} verified records; desired milestone is ${MIN_DESIRED_COUNT}. No filler records will be generated.`);
await mkdir("data", { recursive: true });
await writeFile("data/quotes.json", JSON.stringify(published, null, 2) + "\n", "utf8");
console.log(`Generated ${published.length} verified multilingual public-domain quotations and aphorisms (desired milestone: ${MIN_DESIRED_COUNT}; capacity: ${TARGET_COUNT}).`);
