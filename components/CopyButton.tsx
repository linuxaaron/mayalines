...
export default function CopyButton({ quote, author }: { quote: string; author: string }) {
  ...
  async function handleCopy() { try { await navigator.clipboard.writeText(`“${quote}” — ${author}`); setCopied(true); window.setTimeout(() => setCopied(false), 1600); } catch { setCopied(false); } }
