...
export default async function MostCopiedPage() {
  ...
  return <main className="site-shell"><header className="header"><a className="brand" href="/"><span className="brand-name">MAYALINES</span></a><div className="header-rule"/><nav className="nav" aria-label="Primary navigation"><a href="/">Quotes</a><a href="/popular">Popular</a><a href="/trending">Trending</a><a href="/most-copied" aria-current="page">Most copied</a></nav></header><section className="hero"><p className="eyebrow">MAYALINES · COPY RANKING</p><h1>Most Copied Quotes</h1><p className="hero-copy">Quotes readers copy most often for messages, WhatsApp statuses, captions and social posts.</p></section><section className="library" aria-labelledby="most-copied-heading"><h2 className="section-heading" id="most-copied-heading">Most copied</h2><p className="library-meta">Ranked by total recorded copies</p>{quotes.length ? <QuoteResults quotes={quotes} metricLabel="copies"/> : <div className="empty-ranking">The copy ranking will appear as readers start copying quotes.</div>}</section></main>;
}
