...
const categoryLabels: Record<string, string> = { Love: "Liebe", Wisdom: "Weisheit", Success: "Erfolg", Life: "Leben", Courage: "Mut", Character: "Charakter", Education: "Bildung", Freedom: "Freiheit", Faith: "Glaube", Quotes: "Zitate", Motivation: "Motivation", Inspiration: "Inspiration", Happiness: "Glück", Friendship: "Freundschaft", Science: "Wissenschaft", Philosophy: "Philosophie" };
...
export default function Home() {
  ...
  return <main className="site-shell">
    ...
    <section className="library" id="main-content" aria-labelledby="library-title"><div className="section-heading" id="library-title">Zitatbibliothek</div><p className="library-meta" aria-live="polite">{filteredQuotes.length.toLocaleString("de-DE")} Zitate · {authors.length.toLocaleString("de-DE")} Autoren</p>
      <div className="category-rail" id="categories" aria-label="Zitatkategorien">{categories.map((item) => item === "Alle" ? <button key={item} type="button" onClick={() => handleCategory(item)} aria-pressed={category === item}>{item}</button> : <a key={item} href={`/categories/${slugify(item)}`}>{categoryLabels[item] ?? item}</a>)}</div>
