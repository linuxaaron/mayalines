...
export default function Home() {
  ...
  return <main className="site-shell">
    ...
    <section className="library" id="main-content" aria-labelledby="library-title"><div className="section-heading" id="library-title">Quote library</div><p className="library-meta" aria-live="polite">{filteredQuotes.length.toLocaleString("en-US")} quotes · {authors.length.toLocaleString("en-US")} authors</p>
      ...
      <section className="author-index" id="authors" aria-labelledby="author-title"><div className="section-heading" id="author-title">Discover Quotes by Author</div><p className="hero-copy">Browse quotes by author to find famous words, inspirational thoughts and memorable lines from notable writers, thinkers and public figures. <a href="/authors">Browse all authors</a>.</p><div className="author-row">{authors.slice(0, 30).map((author) => <a href={`/authors/${slugify(author)}`} key={author}>{author}</a>)}</div></section>
      <section className="seo-intro" aria-labelledby="explore-title"><div className="section-heading" id="explore-title">Explore Famous Quotes by Topic</div><p className="hero-copy">Find quotes about love, life, success, motivation, wisdom, courage, happiness, friendship, freedom, philosophy, science, education and more. <a href="/categories">Browse all quote categories</a>. Mayalines organizes a growing library of sourced quotes so you can discover memorable words by topic or author.</p></section>
    </section>
