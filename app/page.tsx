...
export default function Home() {
  ...
  return <main className="site-shell">
    ...
    <header className="header"><a className="brand" href="#top" aria-label="Mayalines home"><img className="brand-mark" src="/mayalines-mark.svg" alt="" aria-hidden="true" /><span className="brand-copy"><span className="brand-name">MAYALINES</span><span className="brand-tagline">WORDS THAT LAST.</span></span></a><div className="header-rule" aria-hidden="true" /><nav className="nav" aria-label="Main navigation"><a href="#top">Home</a><a href="#categories">Categories</a><a href="#authors">Authors</a><a href="#about">About MAYALINES</a></nav></header>
    <section className="hero" id="top" aria-labelledby="page-title"><p className="eyebrow">MAYALINES · QUOTES &amp; THOUGHTS</p><h1 id="page-title">Quotes that last.</h1><p className="hero-copy">Discover famous quotes, inspiring words, thoughts about life and ideas from influential people. Search by author, topic or category.</p><label className="search-box"><span className="sr-only">Search quotes, authors and topics</span><input type="search" value={query} onChange={(event) => handleSearch(event.target.value)} placeholder="Search quotes, authors or topics …" autoComplete="off" /></label></section>
    <section className="library" id="main-content" aria-labelledby="library-title"><div className="section-heading" id="library-title">Quote library</div><p className="library-meta" aria-live="polite">{filteredQuotes.length.toLocaleString("en-US")} quotes · {authors.length.toLocaleString("en-US")} authors</p>
      ...
      <section className="author-index" id="authors" aria-labelledby="author-title"><div className="section-heading" id="author-title">Discover by author</div><div className="author-row">{authors.slice(0, 30).map((author) => <a href={`/authors/${slugify(author)}`} key={author}>{author}</a>)}</div></section>
    </section>
