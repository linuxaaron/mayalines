...
export default function Home() {
  ...
  return <main className="site-shell">
    ...
    <section className="hero" id="top" aria-labelledby="page-title"><p className="eyebrow">MAYALINES · FAMOUS &amp; INSPIRATIONAL QUOTES</p><h1 id="page-title">Famous Quotes, Inspirational Words &amp; Timeless Wisdom</h1><p className="hero-copy">Discover memorable words by author, topic and occasion. Copy a quote for your status, share it with someone, or stay awhile and find something unexpected.</p><label className="search-box"><span className="sr-only">Search quotes, authors and topics</span><input type="search" value={query} onChange={(event) => handleSearch(event.target.value)} placeholder="Search quotes, authors or topics …" autoComplete="off" /></label><div className="hero-links"><a href="/random">✦ Surprise me</a><a href="/popular">Most liked</a><a href="/trending">Trending now</a></div></section>

    <section className="feature-band" aria-labelledby="featured-heading"><div className="section-heading" id="featured-heading">A few words worth keeping</div><div className="feature-grid">{featured.map((item) => <article className="feature-card" key={item.id}><p className="feature-quote">“{item.quote}”</p><p className="quote-author">— {item.author}</p><div className="feature-actions"><PersistentLikeButton quoteId={item.id} author={item.author} /><QuoteActions quote={item.quote} author={item.author} /></div></article>)}</div></section>
    ...
    <section className="quote-of-the-day" aria-labelledby="daily-title"><p className="eyebrow" id="daily-title">QUOTE OF THE DAY</p><blockquote>{quotes[0]?.quote}</blockquote><p className="quote-author">— {quotes[0]?.author}</p><div className="quote-actions"><PersistentLikeButton quoteId={quotes[0]?.id ?? "daily"} author={quotes[0]?.author ?? "MAYALINES"} /><QuoteActions quote={quotes[0]?.quote ?? ""} author={quotes[0]?.author ?? "MAYALINES"} /></div></section>
