...
export default function Home() {
  ...
  const [likedQuotes, setLikedQuotes] = useState<LikeMap>({});

  useEffect(() => {
    try {
      const stored = window.localStorage.getItem(LIKES_STORAGE_KEY);
      if (stored) setLikedQuotes(JSON.parse(stored) as LikeMap);
    } catch {
      setLikedQuotes({});
    }
  }, []);
  ...
  function toggleLike(id: string) {
    setLikedQuotes((current) => {
      const next = { ...current, [id]: !current[id] };
      try {
        window.localStorage.setItem(LIKES_STORAGE_KEY, JSON.stringify(next));
      } catch {
        // Keep the interaction functional for the current session if storage is unavailable.
      }
      return next;
    });
  }
  ...
  return (
    <main className="site-shell">
      ...
      <section className="hero" id="top" aria-labelledby="page-title">
        ...
        <p className="hero-copy">Explore famous quotes, Nietzsche quotes, inspirational words, life quotes and ideas from notable voices. Read them, find your favorites, like the ones that stay with you, and copy them instantly.</p>
      ...
      <section className="library" id="main-content" aria-labelledby="library-title">
        ...
        <div className="quote-grid">
          {visibleQuotes.map((item) => {
            const isLiked = Boolean(likedQuotes[item.id]);
            return (
              <article className="quote-card" key={item.id}>
                ...
                <div className="quote-actions">
                  <button
                    ...
                    aria-label={isLiked ? `Unlike quote by ${item.author}` : `Like quote by ${item.author}`}
