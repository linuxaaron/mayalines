...
export default async function CategoryPage({ params }: { params: Promise<{ category: string }> }) {
  ...
  const title = labels[category] ?? category.replace(/-/g, " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
  const description = descriptions[category] ?? `Discover notable ${title.toLowerCase()} quotes by influential authors on Mayalines.`;
  const publicQuotes = quotesData.filter((quote) => slugify(quote.category) === category && isPublicQuote(quote));
  const seoQuotes = publicQuotes.filter(isSeoIndexable);
  const visibleQuotes = (seoQuotes.length > 0 ? seoQuotes : publicQuotes).slice(0, 60);
  ...
  return <main className="quote-detail">
    ...
    <p className="eyebrow">MAYALINES CATEGORY</p><h1>{title} Quotes</h1><p className="hero-copy">{description}</p><p className="library-meta">{publicQuotes.length.toLocaleString("en-US")} sourced quotes in this category.</p>
    <div className="quote-grid">{visibleQuotes.map((quote) => <article className="quote-card" key={quote.id} lang={quote.language ?? "en"}><div className="quote-mark" aria-hidden="true">“</div><p className="quote-text">{quote.quote}</p><p className="quote-author">— <a href={`/authors/${slugify(quote.author)}`}>{quote.author}</a></p><p className="quote-category"><a href={`/quotes/${quote.slug}`}>Read full quote</a></p><div className="quote-actions"><QuoteActions quote={quote.quote} author={quote.author} quoteId={quote.id} /></div></article>)}</div>
