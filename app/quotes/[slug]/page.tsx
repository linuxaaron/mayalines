...
export function generateStaticParams() {
  return quotesData.filter(isIndexable).map((quote) => ({ slug: quote.slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = `Read this famous quote by ${quote.author}: “${quotePreview.slice(0, 145)}${quotePreview.length > 145 ? "…" : ""}” on Mayalines.`;

  return {
    title: `${quote.author} Quote – “${quotePreview.slice(0, 62)}${quotePreview.length > 62 ? "…" : ""}”`,
    description,
    alternates: { canonical: `/quotes/${quote.slug}` },
    robots: {
      index: true,
      follow: true,
...
export default async function QuotePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!quote || !isIndexable(quote)) notFound();

  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://mayalines.com";
  const authorSlug = slugify(quote.author);
  const categorySlug = slugify(quote.category);
  const quoteUrl = `${siteUrl}/quotes/${quote.slug}`;
  const categoryUrl = `${siteUrl}/categories/${categorySlug}`;
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: `${quote.category} Quotes`, item: categoryUrl },
      { "@type": "ListItem", position: 4, name: `${quote.author} Quote`, item: quoteUrl },
    ],
  };
  ...
  return (
    <main className="quote-detail">
      ...
      <p className="source-note">Explore more <a href={authorUrl}>quotes by {quote.author}</a> or browse <a href={categoryUrl}>{quote.category.toLowerCase()} quotes</a>.</p>
