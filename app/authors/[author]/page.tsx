...
export default async function AuthorPage({ params }: { params: Promise<{ author: string }> }) {
  ...
  const personSchema = {
    ...
    "@type": "Person",
    name: authorName,
    subjectOf: { "@type": "WebPage", url: `${siteUrl}/authors/${slug}`, inLanguage: "en-US" },
  };
  ...
  return <main className="quote-detail">
    ...
    <div className="quote-grid">{indexableQuotes.slice(0, 24).map((quote) => <article className="quote-card" key={quote.id}><div className="quote-mark" aria-hidden="true">“</div><p className="quote-text">{quote.quote}</p><p className="quote-author">— {quote.author}</p><a className="copy-button" href={`/quotes/${quote.slug}`}>VIEW QUOTE</a></article>)}</div>
