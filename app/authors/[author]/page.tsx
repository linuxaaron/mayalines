...
export async function generateMetadata({ params }: { params: Promise<{ author: string }> }): Promise<Metadata> {
  ...
  const count = authorName ? quotesData.filter((quote) => quote.author === authorName && isPublicQuote(quote)).length : 0;
...
export default async function AuthorPage({ params }: { params: Promise<{ author: string }> }) {
  ...
  const visibleQuotes = publicQuotes.slice(0, 24);
  const topics = Array.from(new Set(publicQuotes.map((quote) => quote.category))).sort().slice(0, 8);
  ...
  return <main className="quote-detail">
    ...
    <p className="hero-copy">Explore {publicQuotes.length.toLocaleString("en-US")} sourced quotes by {authorName}. Discover famous, inspirational and timeless words with source information.</p>
    <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 22 }} aria-label="Author quote statistics">
      <span className="copy-button" style={{ color: "var(--muted)" }}>{publicQuotes.length.toLocaleString("en-US")} QUOTES</span>
    ...
    <section aria-labelledby="author-library" style={{ marginTop: 38 }}>
      ...
      <div className="quote-grid">{visibleQuotes.map((quote) => <article className="quote-card" key={quote.id} lang={quote.language ?? "en"}><div className="quote-mark" aria-hidden="true">“</div><p className="quote-text">{quote.quote}</p><p className="quote-author">— {quote.author}</p><p className="quote-category">{quote.category}</p><div className="quote-actions"><QuoteActions quote={quote.quote} author={quote.author} quoteId={quote.id} /></div></article>)}</div>
    </section>
    {publicQuotes.length > visibleQuotes.length && <p className="hero-copy" style={{ marginTop: 24 }}>More sourced quotes by {authorName} are available across the Mayalines library.</p>}
