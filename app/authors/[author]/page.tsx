...
export async function generateMetadata({ params }: { params: Promise<{ author: string }> }): Promise<Metadata> {
  ...
  return { title: authorName ? `Quotes by ${authorName}` : "Quotes by Author", description: authorName ? `Discover remarkable quotes by ${authorName} on Mayalines, with attribution and source information where available.` : "Discover remarkable quotes by author on Mayalines.", alternates: { canonical: authorName ? `/authors/${slug}` : "/authors" }, robots: { index: indexable, follow: true } };
}
...
export default async function AuthorPage({ params }: { params: Promise<{ author: string }> }) {
  ...
  const personSchema = { "@context": "https://schema.org", "@type": "Person", name: authorName, subjectOf: { "@type": "WebPage", url: `${siteUrl}/authors/${slug}` } };
  return <main className="quote-detail">
    ...
    <p className="eyebrow">AUTHOR</p><h1>Quotes by {authorName}</h1>
    <p className="hero-copy">Discover {quotes.length} attributed quotes by {authorName}. {indexableQuotes.length} quotes are currently approved for search engine indexing.</p>
    <div className="quote-grid">{quotes.slice(0, 24).map((quote) => <article className="quote-card" key={quote.id}><div className="quote-mark" aria-hidden="true">“</div><p className="quote-text">{quote.quote}</p><p className="quote-author">— {quote.author}</p><a className="copy-button" href={`/quotes/${quote.slug}`}>VIEW QUOTE</a></article>)}</div>
